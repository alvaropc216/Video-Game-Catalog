    ...
    if request.method == 'POST':
        if login_session['user_id'] == genreToDelete.user_id:
            if genreToDelete.count > 0:
                gamesToDelete = session.query(Game).filter_by(id_genre
                =genre_id).all()
...
def showGames(genre_id):
    ...
    games = gamesToSee, genres = genres, createdgenres = createdgenres)
...
def createGame(genre_id):
    ...
    if request.method == 'POST':
        if 'username' in login_session:
            name = request.form['name']
            existingGame = session.query(Game).filter_by(name
            = name).one_or_none()
            if existingGame is not None:
                ...
                description = description,user_id=login_session['user_id'])
                genreToAdd.count += 1
                session.add(newGame)
        ...
        return render_template('creategame.html', genre = genreToAdd,
        message = message, genres = genres, createdgenres = createdgenres)
...
def viewGame(genre_id, game_id):
    ...
    return render_template('seegame.html',genre = genreToSee, game = gameToSee,
     user_id = user_id, genres = genres, createdgenres = createdgenres)
    ...
    if request.method == 'POST':
        ...
        if gameToEdit.user_id == user_id:
            ...
            if name=="":
                ...
                game = gameToEdit, message = message, genres = genres,
                createdgenres = createdgenres)
            else:
                if description == "":
...
def deleteGame(genre_id,game_id):
    ...
    if request.method == 'POST':
        ...
        game = gameToDelete, genres = genres, createdgenres = createdgenres )
